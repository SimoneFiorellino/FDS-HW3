...
class SVM:
    ...
    def __init__(self, x, y, C=1, tol=0.001, eps=0.001, kernel='linear', sigma=0.45):
        ...
        self.simga = sigma
        kernel_type = {
            'linear': lambda x_i, x_j: cp.sum(cp.dot(x_i, x_j)),
            'gaussian': lambda x_i, x_j: cp.exp(- cp.square(cp.linalg.norm(x_i - x_j)) / 2*(sigma**2))
        }
    ...
    def _check_bounds(self):
        ...
        return cp.flatnonzero(cp.logical_and(self.alpha > 0, self.alpha < self.C))
    ...
    def _get_support_vectors(self):
        ...
        if self.support_vectors_ is None:
            return cp.flatnonzero(self.alpha != 0)
    ...
    def _objective_function(self):
        ...
        result = 0.5 * result - cp.sum(self.alpha[support_vectors_idxs])
        return result
    ...
    def _second_heuristicB(self, j, s_len, arr=None):
        ...
        if arr is None:
            arr = cp.arange(0, s_len)
        for i in cp.roll(arr, cp.random.randint(0, s_len)):
            if self._take_step(i, j):
                return True
        return False
    ...
    def predict(self, x):
        ...
        return int(cp.sign(self.decision_function(x)))
...
x_train = cp.array(train_set.to_numpy())
x_test = cp.array(test_set.to_numpy())

y_train = cp.array(labels_train.to_numpy()).flatten()
y_test = cp.array(labels_test.to_numpy()).flatten()
...
predictions = cp.array(predictions_list)

t = time.time()

print(cp.sum(cp.equal(predictions, y_test)))
print(f'\nTime taken: {t - t0}')
...
# x = cp.hstack([cp.ones((df_x.shape[0], 1)), df_x[["x1","x2"]].values])
# y = df_y["y"].values

# # cp.random.seed(17349)
...
# predictions = cp.array(predictions_list)
# print(predictions)
# print(cp.sum(cp.equal(predictions, y)))